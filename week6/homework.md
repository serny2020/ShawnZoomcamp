...
```
3. Run the Flink job to stream data into Postgres: `docker-compose exec jobmanager ./bin/flink run -py /opt/src/job/session_job.py --pyFiles /opt/src -d` to stream data to postgres.

4. Query Postgres to retrieve the results. 
![alt text](image.png)
